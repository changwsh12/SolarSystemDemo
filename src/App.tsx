...
#app{width:100%;height:100vh;display:flex;flex-direction:column;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft JhengHei","PingFang TC",sans-serif;background:#0a0a1a;color:#fff}
...
.cw{flex:1;position:relative;min-width:0;min-height:0}
.cw canvas{display:block;position:absolute;top:0;left:0;width:100%;height:100%}
...
function rs(){var p=cv.parentElement;if(p&&p.clientWidth>0){cv.width=p.clientWidth;cv.height=p.clientHeight}else{cv.width=window.innerWidth;cv.height=window.innerHeight-60}}
...
function anim(ts){if(lt===0)lt=ts;if(ip)t+=(ts-lt)*sp*.06;lt=ts;if(cv.width>0&&cv.height>0)draw();requestAnimationFrame(anim)}
